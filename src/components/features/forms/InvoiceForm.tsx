...
export function InvoiceForm({ initial, clients, projects, onSave }: InvoiceFormProps) {
        ...
        <div>
          <div className="flex justify-between items-center mb-1.5">
            ...
            <button 
              ...
              onClick={() => { setManualClient(!manualClient); setF({...f, clientId: "", customClient: ""}) }}
            >
              {manualClient ? "Pick from list" : "Type manually"}
            </button>
          </div>
          {manualClient 
            ? <Input placeholder="Enter client name..." value={f.customClient} onChange={(e) => setF({...f, customClient: e.target.value})} />
            : <CustomSelect 
                value={f.clientId} 
        ...
        <div>
          <div className="flex justify-between items-center mb-1.5">
            ...
            <button 
              type="button" 
              className="text-[10px] font-black text-[#a78bfa] uppercase tracking-widest hover:underline" 
              onClick={() => { setManualProject(!manualProject); setF({...f, projectId: "", customProject: ""}) }}
            >
              {manualProject ? "Pick from list" : "Type manually"}
            </button>
          </div>
          {manualProject 
            ? <Input placeholder="Enter project title..." value={f.customProject} onChange={(e) => setF({...f, customProject: e.target.value})} />
            : <CustomSelect 
                value={f.projectId} 
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button 
            ...
            onClick={() => setF({...f, items: [...f.items, { description: "", qty: "1", rate: "" }]})}
          >
            <FiPlus /> Add Item
          </button>
        </div>
        <div className="space-y-3">
          {f.items.map((item: any, i: number) => (
            <div key={i} className="flex gap-3 items-start animate-in fade-in slide-in-from-left-2 duration-300">
              ...
              <button 
                type="button" 
                className="p-3.5 bg-[#ef4444]/10 text-[#ef4444] rounded-lg hover:bg-[#ef4444]/20 transition-all" 
